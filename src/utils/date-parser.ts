...
export function parseDate(input?: string): ParsedDate {
  ...
  if (lowerInput === "tomorrow") {
    return {
      type: "single",
      dates: [today.add(1, "day").format("YYYY-MM-DD")],
    };
  }
  ...
  if (lowerInput.includes("-")) {
    ...
    if (parts.length === 2) {
      ...
      if (startDay !== undefined && endDay !== undefined) {
        return {
          type: "range",
          dates: generateDayRange(startDay, endDay, today),
        };
      }
    }
...
function parseSpecificDate(input: string): string | null {
  ...
  for (const format of formats) {
    ...
    if (parsed.isValid()) {
      // If no year in format, use current year (or next year only if date is more than 30 days in the past)
      if (!format.includes("YYYY")) {
        let date = parsed.year(currentYear);
        // Only jump to next year if the date is more than 30 days ago
        // This allows checking recent past dates while still handling future dates correctly
        if (date.isBefore(today, "day") && today.diff(date, "day") > 30) {
...
function generateDayRange(
  ...
  const daysToGenerate =
    endDay >= startDay ? endDay - startDay + 1 : 7 - startDay + endDay + 1;
